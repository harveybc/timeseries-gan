...
This pre-trained model is now a sub-component of the main GAN Generator. It is set to `trainable=True` during GAN training.
- **Input (as part of the larger GAN Generator context)**: The VAE decoder itself expects multiple input streams, which are prepared by the encapsulating GAN Generator:
  - `decoder_input_z_seq`: Latent sequences `(batch_size, 18, 32)`. These are internally generated by the GAN Generator's own preceding layers (e.g., from noise via BiLSTM/Conv1D).
  - `decoder_input_h_context`: Context vector `(batch_size, 64)`. This is dynamically populated by the GAN Generator using the synthetic features (excluding TIs) generated at the *previous* timestep.
  - `decoder_input_conditions`: Conditional features vector `(batch_size, 10)`. This is populated by the GAN Generator using cyclical date/time features (e.g., day of month, day of week, hour of day) for the *current* timestep. Remaining dimensions are zero-padded.
- **Output (from the `reconstruction_out` layer)**: The VAE decoder's primary output layer, named `reconstruction_out`, produces a tensor of shape `(batch_size, 23)`. These 23 features are the point-wise (non-sequential) reconstructions of the target features for the current timestep. The `GeneratorPlugin` uses these 23 base features to then calculate technical indicators and assemble the full 57-feature output, including date features derived from a datetime sequence.
  The 23 output features correspond in order to the `cvae_target_feature_names` list defined in `app/config.py`:
  1.  `"OPEN"`
  2.  `"LOW"`
  3.  `"HIGH"`
  4.  `"vix_close"`
  ...
  22. `"CLOSE_30m_tick_7"`
  23. `"CLOSE_30m_tick_8"`
  These are the core, non-calculable features. The `GeneratorPlugin` is responsible for subsequently calculating technical indicators and adding cyclical date/time features (day of month, day of week, hour of day) using the specific datetime of the generated tick to complete the full feature set for the synthetic output.
...
- **Generator**: The GAN Generator is a new composite Keras model. It includes:
    1.  An internal sub-model that processes noise (from the Feeder plugin) to produce the `decoder_input_z_seq`.
    2.  The pre-trained VAE Decoder (loaded from `phase_4_3_cnn_small_decoder_model.keras`), which is set to `trainable=True` to allow fine-tuning.
    This composite generator takes initial noise, the previous timestep's output (for context), and current timestep's date/time conditions to iteratively produce a sequence.
- **Discriminator**: Custom Conv1D/LSTM architecture that processes full 57-feature sequences.
- **Training**: Adversarial training improves the quality of the composite generator's outputs.
...
Initial Noise/Seed → Feeder Plugin → Composite GAN Generator → Discriminator → Evaluation
        ↓                  ↓                     ↓                     ↓               ↓
Date/Time Info     Noise & Initial      (Internal Z-gen → VAE Decoder)  GAN Training  Metrics
(for conditions)   Conditions Prep.     + Iterative Context Update      Improvement   Computation
                                        + TI Calc. post-generation
```

1.  **Feeder Plugin**: Generates initial noise to seed the GAN generator and provides date/time information for conditional inputs.
2.  **Composite GAN Generator**:
    - Internally generates a latent sequence (`decoder_input_z_seq`) from the feeder's noise.
    - Iteratively calls the pre-trained (but trainable) VAE Decoder, feeding it the generated latent sequence, a context vector derived from the previous step's output, and conditional date/time features for the current step.
    - After generating the base feature sequence, technical indicators are calculated.
3.  **Discriminator**: Distinguishes real vs synthetic full-feature sequences for adversarial training
